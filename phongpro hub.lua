...
xpcall(function()
    if not isfolder("phong scripts/ID/" .. game.GameId) then
        if syn then
            makefolder("phong scripts/ID/" .. game.GameId)
            writefile("phong scripts/ID/" .. game.GameId .. "/" .. game.Players.LocalPlayer.Name .. ".lua",
                game:GetService("HttpService"):JSONEncode(SettingsGameData));
        else
            makefolder("phong scripts")
            makefolder("phong scripts/ID")
            makefolder("phong scripts/ID/" .. game.GameId)
            writefile("phong scripts/ID/" .. game.GameId .. "/" .. game.Players.LocalPlayer.Name .. ".lua",
                game:GetService("HttpService"):JSONEncode(SettingsGameData));
        end
    ...
    SettingsGameData = game:GetService("HttpService"):JSONDecode(readfile(
        "phong scripts/ID/" .. game.GameId .. "/" .. game.Players.LocalPlayer.Name .. ".lua"));
end, function()
end)
...
local phong = loadstring(game:HttpGet(
...
local Window = phong:CreateWindow({
    Title = "phong Hub | Premium",
    SubTitle = "by phong",
    TabWidth = 160,
        ...
        while wait() do
            if AutoObservation then
                if game:GetService("Players").LocalPlayer.VisionRadius.Value >= 3000 then
                    local ahasd = Notification.new("success", "phong Hub", "Observation Range : Max")
                    ...
                    elseif game:GetService("ReplicatedStorage").Remotes.CommF_:InvokeServer("CDKQuest", "Progress")["Finished"] ==
                        ...
                        if tonumber(game:GetService("ReplicatedStorage").Remotes.CommF_:InvokeServer("CDKQuest",
                            ...
                            if (CFrame.new(-4602.5107421875, 16.446542739868164, -2880.998046875).Position -
                                ...
                                wait(.7)
                                game:GetService("ReplicatedStorage").Remotes.CommF_:InvokeServer("CDKQuest",
                                    "BoatQuest", workspace.NPCs:FindFirstChild("phong Boat Dealer"), "Check")
                                wait(.5)
                                game:GetService("ReplicatedStorage").Remotes.CommF_:InvokeServer("CDKQuest",
                                    "BoatQuest", workspace.NPCs:FindFirstChild("phong Boat Dealer"))
                            end
                            wait(1)
                            repeat
                                wait()
    ...
    spawn(function()
        while fask.wait(.5) do
            if AutoPullFuly then
                ...
                local notif = Notification.new("success", "phongBF", "Can't Find Because not night or mirage") -- Args(<string> Type, <string> Heading, <string> Body, <boolean?> AutoRemoveNotif, <number?> AutoRemoveTime, <function?> OnCloseFunction)
...
function WebHookLog:WebHookKaiTanSend(WebHookUrl)
    ...
    local Embeds = {{
        ...
        ["footer"] = {
            ...
            ["icon_url"] = "https://cdn.discordapp.com/attachments/1014897847059099739/1038449526865932298/Logo-phong1.png"
        }
    }}
    ...
    if _G.SendWebHookPing then
        Message = {
            ['username'] = "phongHub Webhook",
            ["avatar_url"] = "https://cdn.discordapp.com/attachments/1014897847059099739/1038449526865932298/Logo-phong1.png",
            ["content"] = "@everyone",
            ["embeds"] = Embeds
        }
    else
